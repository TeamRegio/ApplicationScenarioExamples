...
int main(int argc, char *argv[]){
  ...
  //double entropycutoff1 = 0.2;
  double entropycutoff1 = 0.1;
  double entropycutoff2 = 0.1;

  //GLOBAL GENOMIC BACKGROUND
  const double gc_content = 0.5;
  const double at_content = 1 - gc_content;

  cout << "#/Lambda=" << lambda << "\t/Regression_slope=" << slope << "\t/Regression_intercept=" << intercept << "\t/Entropy_core_cutoff=" << entropycutoff1 << "\t/Entropy_general_cutoff=" << entropycutoff2 << "\t/GC_content=" << gc_content << "\t/Pseudocount=maximal_row_counts/counts_in_row\n";
  ...
  for(int f = 0; f <= factors; f++){
    ...
    //ENTROPY CUTOFF
    effectivelength = 0;
    double previousent, nextent;
    int info[motiflength[f]];
    
    for(int p = 0; p < motiflength[f]; p++){
      
      if(p == 0){previousent = 0;}
      else{previousent = entropy[f][p-1];}
      if(p == motiflength[f] - 1){nextent = 0;}
      else{nextent = entropy[f][p+1];}
      
      //      if((previousent < 0.15)&&(nextent < 0.15)){
      if((previousent < entropycutoff1)&&(nextent < entropycutoff1)){
	if(entropy[f][p] < entropycutoff1){
	  info[p] = 0;
	  matrix[f][p][A] = 0;
	  matrix[f][p][T] = 0;
	  matrix[f][p][C] = 0;
    ...
    n = motiflength[f] - 1;
    core = 0;
    for(int p = motiflength[f] - 1; p > pos; p--){
      if(info[p] == 1){
	core = 1;
      }
      if(core == 0){
