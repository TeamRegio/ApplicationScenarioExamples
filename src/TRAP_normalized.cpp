...
//{
//#pragma omp parallel for  private(dE_forward, dE_compl, illegalBase, BASE, length, product, P_bound_F, P_bound_C, P_combined, totalvalids) num_threads(5)
//.............

    //LOOP OVER FACTORS
    for(int f = 0; f <= factors; f++){
	length = motiflength[f];
	...
	if(outputtype == 0){
		if (totalvalids > 0){
			factors_result[f] = (P_combined/totalvalids)/length;
		}
	}else{
