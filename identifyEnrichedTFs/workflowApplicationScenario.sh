...
pathToBedtools=$3 #path to bedtools
genome=$4 # genome in fasta format
REMs=$5 #CSV file from EpiRegio
outputDir=$6 #user defined output dir
pvalue=$7 #pvalue threshold for PASTAA
...
${pathToBedtools}bedtools getfasta -name -fi ${genome} -bed ${outputDir}REMs.bed -fo ${outputDir}REMs_.fa #determine fasta seq
...
python3 ${pathToRepo}rankingPASTAA_.py  ${REMs} ${outputDir}REMs_.fa ${outputDir}ranking.txt ${outputDir}REMs.fa
...
python3 ${pathToRepo}fdrPASTAAResult_.py ${outputDir}PASTAA_output.txt ${outputDir}PASTAA_result.txt ${pvalue}
